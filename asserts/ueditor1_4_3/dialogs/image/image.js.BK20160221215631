    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                // add by edwin
                // start
                var deleteAction = editor.getOpt('deleteActionName');
                if(deleteAction) {
                    var ul = $(li.parentNode);
                    var deleteBtn = $('#deleteBtn');
                    var files = [];
                    ul.children('li.selected').each(function (i, o) {
                        files.push($(li).children('img').attr('_src'));
                    });
                    if (files.length > 0) {
                        deleteBtn.show();
                        deleteBtn.unbind('click');
                        deleteBtn.on('click', function () {
                            var url = editor.getActionUrl(deleteAction),
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS') {
                                ...
                                // start
                                if(json.space){
                                    var spaceTxt = $('#spaceTxt');
                                    spaceTxt.html(json.space);
                                    spaceTxt.show();
                                }
                                // end
        ...
        pushData: function (list) {
            var i, item, img, icon, _this = this,
                ...
                urlPrefixPreview = editor.getOpt('imageManagerUrlPrefixPreview');
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', urlPrefixPreview + list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
